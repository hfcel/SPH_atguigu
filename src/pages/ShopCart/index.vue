  ...
  <div class="cart">
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllChecked"  @change="updateAllCheck"/>
        <span>全选</span>
      </div>
      <div class="option">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      handler:throttle(async function(type,curNum,cart){
      ...
      async updateChecked(cart,e){
        try {
          // console.log(cart)
          // let curCheck=e.target.isChecked?'1':'0'
          let curCheck='1'
          ...
          await this.$store.dispatch('updateCheckedByid',{skuId:cart.skuId,isChecked:curCheck})
          this.getData()
        } catch (error) {
          alert(error.message)
        }
      ...
      async updateAllCheck(e){
        try {
          await this.$store.dispatch('updateAllCheck',e.target.checked)
          this.getData()
        } catch (error) {
          alert(error.message)
        }
    ...
    computed:{
      ...
      totalPrice(){
        ...
        for(let i=0;i<this.cartInfoList.length;i++){
          let cur=this.cartInfoList[i].skuNum*this.cartInfoList[i].skuPrice;
          res+=cur;
        }
        return res
      },
